...
- Dealer submissions first enter **Awaiting PM Review**. The PM Manager or PM Admin chooses the priority, records decision remarks visible to the dealer, and selects **Dial-A** or **In house**, then clicks **Assign**. The fields lock and the buttons become **Proceed** and **Undo Assigned**. Undo Assigned reopens the decision fields.
- **Proceed** releases the workflow and locks assignment against PM changes. Only the system **Administrator** can undo it afterward, using their own password from the request page. Undo retains saved work and attachments and pauses workflow actions until PM assigns and proceeds again. PM Admin has PM Manager permissions, not system Administrator override permissions.
- In house shows Work Order and the next workflow steps only after **Proceed**. Work Order changes and Completion Report submission are blocked until then. Dial-A also receives and can work on the request only after Proceed. Existing requests remain in their current workflow.
