...
<div class="role-grid">
    ...
    @foreach(['pm_manager' => 'PM Manager', 'pm_admin' => 'PM Admin'] as $role => $label)
        <article class="panel role-card"><div class="role-card-head"><span class="role-icon manager-role">{{ $role === 'pm_admin' ? 'A' : 'M' }}</span><div><h2>{{ $label }}</h2><p>{{ $counts[$role] ?? 0 }} account</p></div></div><ul><li>Review requests and update priority with password confirmation</li><li>Assign requests to Dial-A or In house and manage in-house work</li><li>Manage dealer records and publish completed request reports</li></ul></article>
    @endforeach
